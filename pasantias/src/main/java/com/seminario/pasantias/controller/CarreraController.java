...
public class CarreraController {
    ...
    public GenericResponse updateCarrera(@RequestBody CarreraRequest request) {
        log.debug("Update carrera request. id={}, nombre={}", request.getId(), request.getNombre());
        ...
        try {
            ...
            log.debug("Carrera a actualizar. id={}, nombre={}", carrera.getId(), carrera.getNombre());
            carreraService.updateCarrera(carrera);
            log.debug("Carrera actualizada exitosamente. id={}", carrera.getId());
            response.setCode(0);
            response.setMessage(null);
        } catch (Exception e) {
            log.warn("Error al actualizar carrera. id={}, msg={}", request.getId(), e.getMessage());
            response.setCode(-1);
            response.setMessage(e.getMessage());
        }
        return response;
